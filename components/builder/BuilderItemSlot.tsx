...
export const BuilderItemSlot: React.FC<BuilderItemSlotProps> = ({ item, index, onRemove, onDragStart, onDragOver, onDrop }) => {
    if (item) {
        return (
            <div className="aspect-square rounded-[1.5rem] border-2 border-lol-gold bg-[#080808] shadow-glow-gold flex items-center justify-center relative group transition-all duration-300">
                <button
                    type="button"
                    draggable
                    ...
                    className="w-full h-full relative cursor-grab active:cursor-grabbing p-1.5 focus:outline-none focus:ring-2 focus:ring-lol-gold rounded-[1.2rem] block border-none bg-transparent appearance-none text-left"
                >
    ...
    return (
        <button
            ...
            onDragOver={onDragOver}
            onDrop={(e) => onDrop(e, index)}
            className="aspect-square rounded-[1.5rem] border-2 border-white/5 bg-[#080808]/50 border-dashed hover:border-white/20 flex items-center justify-center relative group transition-all duration-300 w-full"
        >
